/**
  ...
  *          mounted on OpenX07V-C development board.
  ...
  [..]
   ...
   (#) This driver supports the AM-240320L8TNQW00H (LCD_ILI9320) and
  AM240320D5TOQW01H (LCD_ILI9325) LCD mounted on MB785 daughter board

   (#) The ILI9320 and ILI9325 components driver MUST be included with this
  driver.
   ...
   (#) Display on LCD
       (++) Clear the hole LCD using yhe BSP_LCD_Clear() function or only one
  specified string line using the BSP_LCD_ClearStringLine() function.
       (++) Display a character on the specified line and column using the
  BSP_LCD_DisplayChar() function or a complete string line using the
  BSP_LCD_DisplayStringAtLine() function.
       (++) Display a string line on the specified position (x,y in pixel) and
  align mode using the BSP_LCD_DisplayStringAtLine() function.
       (++) Draw and fill a basic shapes (dot, line, rectangle, circle, ellipse,
  .. bitmap, raw picture) on LCD using a set of functions.
...
void BSP_LCD_DisplayChar(uint16_t Xpos, uint16_t Ypos, uint8_t Ascii) {
    LCD_DrawChar(
        Xpos, Ypos, &DrawProp.pFont->table[(Ascii - ' ') * DrawProp.pFont->Height * ((DrawProp.pFont->Width + 7) / 8)]);
}
...
void BSP_LCD_DisplayStringAt(uint16_t Xpos, uint16_t Ypos, uint8_t* pText, Line_ModeTypdef Mode) {
    ...
    while ((*pText != 0)
           & (((BSP_LCD_GetXSize() - (counter * DrawProp.pFont->Width)) & 0xFFFF) >= DrawProp.pFont->Width)) {
