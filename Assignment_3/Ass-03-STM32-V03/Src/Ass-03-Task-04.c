...
//        	            win_ptr = 0;
//        	        }
//        	        Window_buffer[win_ptr] = (double)((double)142.0 * ((double)ADC_Value[begin] / (double)4096.0));//YSIZE * ((double) avg / (4096.0));  // TODO Map to the max
...
//        	    //printf("Scale is %lf\n", scale);
//        	    safe_printf("pos %3d, buffer %3.2lf, max %3.2lf, scale %2.2lf, output %4d\n",win_ptr, Window_buffer[win_ptr], max, scale, ((int) (Window_buffer[win_ptr] * scale)));
//
//        	    int last_xpos = 0;
//        	    int last_ypos = 0;
...
//        }
                   ...
                   for(int pos = 5; pos < 250; pos+=5){
                	 	if(Window_buffer[pos] - Window_buffer[pos - 5] > delta_bpm){
                	 		...
                	 		//safe_printf("%d, %d, %d\n", Window_buffer[pos], Window_buffer[pos - 5], Window_buffer[pos] - Window_buffer[pos - 5]);
                	 	}
                   }
                   last_bpm++;
                   if(last_bpm >= 50){
