...
int read_coefficients(int* coeff_num_p, double** coeff_values_p, char* filename) {
    ...
    if ((size_file - sizeof(int)) % coeff_num != 0) {  // If remaining bytes doesn't add up TODO this should be
                                                       // (((size_file - 4)/coeff_num) == sizeof(double))
    ...
    if (fread(coeff_values, 8, coeff_num, file_p) != coeff_num)
        ...
        for (int i = 0; i < coeff_num; i = i + 3) {  // Loop in groups of 3
            for (int k = i; k < i + 3; k++) {        // For each group of 3
                if (k < coeff_num) {                 // Check we haven't accidentally passed further than the array
                    if (coeff_values[k] < 0)
                        printf("%3s %lf", " ", coeff_values[k]);  // If negative use less spaces
                    else
                        printf("%4s %lf", " ", coeff_values[k]);  // If positive use more spaces
                }
            }
