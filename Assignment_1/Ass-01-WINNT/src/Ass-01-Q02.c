...
int read_pcm_wavefile(pcm_wavefile_header_t* header_p, char** data_p, char* filename) {
    ...
    if (fread(head_data.ChunkID, ui8_t_size, 4, file_p) != 4)
        printf("%3s ERROR : fread : %s\n", " ", "head_data.ChunkID");
    ...
    // Read Chunk Size
    // Compare to expected value
    if (fread(&head_data.ChunkSize, ui32_t_size, 1, file_p) != 1)
        printf("%3s ERROR : fread : %s\n", " ", "&head_data.ChunkSize");

    // Read Format
    // Compare to expected value
    if (fread(head_data.Format, ui8_t_size, 4, file_p) != 4)
        printf("%3s ERROR : fread : %s\n", " ", "head_data.Format");
    if (strncmp(fmt_string, (char*) head_data.Format, 3) != 0) {
        printf("%3s ERROR: Incorrect Format expected: %s got: %c%c%c%c\n",
               " ",
               fmt_string,
               head_data.Subchunk1ID[0],
               head_data.Subchunk1ID[1],
               head_data.Subchunk1ID[2],
               head_data.Subchunk1ID[3]);
        return -1;
    }

    // Read Sub Chunk ID 1
    // Compare to expected value
    if (fread(head_data.Subchunk1ID, ui8_t_size, 4, file_p) != 4)
    ...
    // Compare to expected value
    if (fread(&head_data.Subchunk1Size, ui32_t_size, 1, file_p) != 1)
        printf("%3s ERROR : fread : %s\n", " ", "&head_data.Subchunk1Size");
    // TODO Why am i doing this?
    if (head_data.Subchunk1Size != 16) {
        if (fread(&head_data.AudioFormat, ui16_t_size, 1, file_p) != 1)
    ...
    // Compare to expected value
    if (fread(&head_data.NumChannels, ui16_t_size, 1, file_p) != 1)
    ...
    // Read SampleRate
    if (fread(&head_data.SampleRate, ui32_t_size, 1, file_p) != 1)
        printf("%3s ERROR : fread : %s\n", " ", "&head_data.SampleRate");

    // Read ByteRate
    if (fread(&head_data.ByteRate, ui32_t_size, 1, file_p) != 1)
        printf("%3s ERROR : fread : %s\n", " ", "&head_data.ByteRate");

    // Read Block Align
    if (fread(&head_data.BlockAlign, ui16_t_size, 1, file_p) != 1)
        printf("%3s ERROR : fread : %s\n", " ", "&head_data.BlockAlign");

    // Read Bits Per Sample
    if (fread(&head_data.BitsPerSample, ui16_t_size, 1, file_p) != 1)
        printf("%3s ERROR : fread : %s\n", " ", "&head_data.BitsPerSample");

    // Read Sub Chunk ID 2
    // Compare to expected value
    if (fread(head_data.Subchunk2ID, ui8_t_size, 4, file_p) != 4)
        printf("%3s ERROR : fread : %s\n", " ", "head_data.Subchunk2ID");
    if (strncmp(schunk2id_string, (char*) head_data.Subchunk2ID, 3) != 0) {
        printf("%3s ERROR: Incorrect Format expected: %s got: %c%c%c%c\n",
               " ",
               schunk2id_string,
               head_data.Subchunk2ID[0],
               head_data.Subchunk2ID[1],
               head_data.Subchunk2ID[2],
               head_data.Subchunk2ID[3]);
        return -1;
    }

    // Read Sub Chunk 2 Size
    // Compare to expected value
    if (fread(&head_data.Subchunk2Size, ui32_t_size, 1, file_p) != 1)
    ...
    *data_p = (char*) calloc(head_data.Subchunk2Size, sizeof(char));
    if (fread(*data_p, sizeof(char), head_data.Subchunk2Size, file_p) != head_data.Subchunk2Size)
        printf("ERROR: Reading data\n");
    else
        printf("Success reading data\n");
 ...
 * 			Success returns 0; Failure returns -1
 */
int write_pcm_wavefile(pcm_wavefile_header_t* header_p,
                       char* data,
                       char* filename_output) { /* I changed the signature from *filename to *filename_output */
    ...
    if (fwrite(head_data.ChunkID, 4, 1, file_p) != 1)
        printf("%3s ERROR : fwrite : %s\n", " ", "head_data.ChunkID");  // Write header to file
    if (fwrite(&head_data.ChunkSize, 4, 1, file_p) != 1)
        printf("%3s ERROR : fwrite : %s\n", " ", "&head_data.ChunkSize");  // Write header to file
    if (fwrite(&head_data.Format, 4, 1, file_p) != 1)
        printf("%3s ERROR : fwrite : %s\n", " ", "&head_data.Format");  // Write header to file

    if (fwrite(&head_data.Subchunk1ID, 4, 1, file_p) != 1)
        printf("%3s ERROR : fwrite : %s\n", " ", "&head_data.Subchunk1ID");  // Write header to file
    if (fwrite(&head_data.Subchunk1Size, 4, 1, file_p) != 1)
        printf("%3s ERROR : fwrite : %s\n", " ", "&head_data.Subchunk1Size");  // Write header to file
    if (fwrite(&head_data.AudioFormat, 2, 1, file_p) != 1)
        printf("%3s ERROR : fwrite : %s\n", " ", "&head_data.AudioFormat");  // Write header to file
    if (fwrite(&head_data.NumChannels, 2, 1, file_p) != 1)
        printf("%3s ERROR : fwrite : %s\n", " ", "&head_data.NumChannels");  // Write header to file
    if (fwrite(&head_data.SampleRate, 4, 1, file_p) != 1)
        printf("%3s ERROR : fwrite : %s\n", " ", "&head_data.SampleRate");  // Write header to file
    if (fwrite(&head_data.ByteRate, 4, 1, file_p) != 1)
        printf("%3s ERROR : fwrite : %s\n", " ", "&head_data.ByteRate");  // Write header to file
    if (fwrite(&head_data.BlockAlign, 2, 1, file_p) != 1)
        printf("%3s ERROR : fwrite : %s\n", " ", "&head_data.BlockAlign");  // Write header to file
    if (fwrite(&head_data.BitsPerSample, 2, 1, file_p) != 1)
        printf("%3s ERROR : fwrite : %s\n", " ", "&head_data.BitsPerSample");  // Write header to file

    if (fwrite(&head_data.Subchunk2ID, 4, 1, file_p) != 1)
        printf("%3s ERROR : fwrite : %s\n", " ", "&head_data.Subchunk2ID");  // Write header to file
    if (fwrite(&head_data.Subchunk2Size, 4, 1, file_p) != 1)
    ...
    if (fwrite(data, head_data.Subchunk2Size, 1, file_p) != 1)
        printf("%3s ERROR : fwrite : %s\n", " ", "head_data.Subchunk2Size");
    else
