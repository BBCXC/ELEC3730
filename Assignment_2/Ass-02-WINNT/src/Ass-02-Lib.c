...
double parsePro(){
	...
	while(Get_Formula_c() == 'x' || Get_Formula_c() == '*'){
		Increment_Formula();
...
double parseFactor(){
	 printf("Line Number %i\n", __LINE__);
	 printf("Next Symbol %c", Get_Formula_c());
	if(Get_Formula_c() >= '0' && Get_Formula_c() <= '9'){
		 printf("Line Number %i\n", __LINE__);
		return parseNumber();
	}
	else if(Get_Formula_c() == '-'){
		return parseNumber();
	}
	else if(Get_Formula_c() == '+'){
		return parseNumber();
	...
	//asin acos atan
		...
		else if(Get_Formula_c() == 'n'){
			Increment_Formula();
			if(Get_Formula_c() == 's'){
				Increment_Formula();
				double temp = Get_Prev_ans();
				return temp;
			}
		}
	}
	//exp
	else if(Get_Formula_c() == 'e'){
		Increment_Formula();
		if(Get_Formula_c() == 'x'){
			Increment_Formula();
			if(Get_Formula_c() == 'p'){
...
double parseNumber(){
	...
	while(Get_Formula_c() >= '0' && Get_Formula_c() <= '9'){
		number = number * 10;
		number = number + (int)(Get_Formula_c() - '0');
		Increment_Formula();
	}
	...
	if(Get_Formula_c() == '.'){
		...
		if(Get_Formula_c() >= '0' && Get_Formula_c() <= '9'){
			...
			while(Get_Formula_c() >= '0' && Get_Formula_c() <= '9'){
				weight = weight / 10.0;
				double scaled = (int)(Get_Formula_c() - '0') * weight;
