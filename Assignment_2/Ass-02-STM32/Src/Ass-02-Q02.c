...
void CalculatorInit() {
    ...
    if (Get_System() == 1)
        printf("%sSYSTEM_INFO:%s Equation currently has %i memory free of total %i\n",
               SYS_M,
               DEFAULT_COLOUR_M,
               Get_Size() - Get_Pos(),
               Get_Size());
...
int draw_numpad() {
    ...
    for (int i = 0; i < 21; i++) {
        if (draw_item(i, 0, LCD_COLOR_BLACK, LCD_COLOR_WHITE) == 0) {
            ...
            if (Get_Debug() == 1)
                printf("%sDEBUG_INFO Numpad:%s item %s, cell_number %i, grid_space %i\n",
                       DEBUG_M,
                       DEFAULT_COLOUR_M,
                       Get_Item(i),
                       i,
    ...
    if (BSP_TP_GetDisplayPoint(&display) == 0) {
        ...
        if (button_debounce >= 50 && holding == 0) {
            ...
            if (touch_pos != 100) {
                ...
                else if ((strcmp(Get_Item(touch_pos), ">") == 0) || (strcmp(Get_Item(touch_pos), "<") == 0)) {
                    // Switch display mode
                    display_mode++;
                    if (display_mode > 1) {
                        display_mode = 0;
                ...
                // If the equals was selected
                    ...
                    if (Get_System() == 1)
                        printf(
                            "%sSYSTEM_INFO:%s Previous answer saved as %lf\n", SYS_M, DEFAULT_COLOUR_M, Get_Prev_ans());
...
int LCD_Cell_Highlight(int status, int item_number, int display_mode) {
    ...
    if (cell_number < 0 || cell_number > 20) {
        ...
        // Highlight is currently off
            ...
            if (LCD_Cell_Colour(x_min, x_max, y_min, y_max, LCD_COLOR_BLACK, LCD_COLOR_YELLOW) != 0) {
                printf("%sERROR:%s Could not clear cell\n", ERROR_M, DEFAULT_COLOUR_M);
            }
            if (draw_item(cell_number, offset, LCD_COLOR_BLACK, LCD_COLOR_YELLOW) != 0)
                printf("%sERROR:%s Could not redraw symbol\n", ERROR_M, DEFAULT_COLOUR_M);
            return 0;
        }
        else {
