...
char *Get_Input_Str(int position){
	return(equation.input[position]);
}
...
void Free_Input_Str(int position){
	free(equation.input[position]);
}
...
char Get_Formula_c_pos(int position){
	return(output.formula[position]);
}
...
int reallocate_memory(){
	...
	for(int i=0; i<equation.pos; i++){
		if(Get_Debug() == 1)printf("%sDEBUG_INFO:%s Equation.Input[%i] Before %s length %i\n", DEBUG_M, DEFAULT_COLOUR_M,
	...
	for(int i=0; i<equation.pos; i++){
		if(Get_Debug() == 1)printf("%sDEBUG_INFO:%s Equation.Input[%i] After %s  length %i\n", DEBUG_M, DEFAULT_COLOUR_M,
								   i, equation.input[i], strlen(equation.input[i]));
	}
...
int Input_append(char *item){
	...
	if(equation.pos == 0){
		...
		if(Get_Debug() == 1 || Get_System() == 1)printf("%sDEBUG_INFO:%s formula contains %s\n", DEBUG_M, DEFAULT_COLOUR_M, output.formula);
	}
