...
int draw_axisnum() {
    ...
    for (int i = 0; i <= MAX_AXIS_NUM; i++) {
        ...
        BSP_LCD_DisplayStringAt((i * x_spacing), x_axis + (0.5 * CHAR_HEIGHT), (uint8_t*) num_str, CENTER_MODE);
    }
    // Print Y AXIS values
    ...
    for (int i = 0; i <= MAX_AXIS_NUM; i++) {
        ...
        snprintf(num_str, 50, "%g", printnum);
        BSP_LCD_DisplayStringAt(y_axis - CHAR_HEIGHT, (i * y_spacing), (uint8_t*) num_str, CENTER_MODE);
    }
